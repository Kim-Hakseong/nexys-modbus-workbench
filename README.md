...
| **내장 시뮬레이터** | 물리 장비 없이 테스트 가능한 내장 슬레이브 — **Modbus TCP + RTU 시리얼** (FC01~06,15,16, 값 편집·자동 변화, 데이터 공유) |
...
**RTU(시리얼) 슬레이브 모드**: 시뮬레이터 창에서 모드를 "RTU 시리얼"로 바꾸면
지정한 COM 포트에서 RTU 슬레이브로 응답한다. 같은 PC에서 마스터와 함께 테스트하려면
가상 COM 포트 쌍이 필요하다 (Windows: [com0com](https://com0com.sourceforge.net/)으로
COM3↔COM4 쌍 생성 → 슬레이브를 COM4에 시작, [연결...]에서 RTU COM3으로 연결).
서로 배선한 USB-RS485 컨버터 2개로도 가능하다. TCP/RTU 슬레이브는 같은 데이터
저장소를 공유하므로 어느 쪽으로 붙어도 같은 값이 보인다.
